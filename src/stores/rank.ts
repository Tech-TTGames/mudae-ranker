...
export const useRankStore = defineStore('rank', () => {
    ...
    // --- Getters ---
    const hasActiveMatch = computed(() => currentMatch.value[0] !== null && currentMatch.value[1] !== null);
    ...
    function nextPlacementMatch(): boolean {
        ...
        const activeRoster = characterStore.unskippedCharacters.filter(c =>
            c.id !== target.id && !placementState.value.queue.some(qc => qc.id === c.id)
        );
    ...
    function nextEndlessMatch(): boolean {
        ...
        let candidates = allCandidates.filter(c =>
            !recentMatchups.value.includes(getMatchupSignature(charA, c))
        );
    ...
    function resolveMatch(winnerIndex: 0 | 1 | -1) {
        ...
        char1.mu = newTeam1[0].mu;
        char1.sigma = newTeam1[0].sigma;
        char1.osRating = newTeam1[0];
        char1.score = ordinal(newTeam1[0]);

        char2.mu = newTeam2[0].mu;
        char2.sigma = newTeam2[0].sigma;
        char2.osRating = newTeam2[0];
        char2.score = ordinal(newTeam2[0]);
